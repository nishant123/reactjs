      ...
      <Collapse navbar>
        <Nav className="ml-auto" navbar>
          <InputGroup>
            ...
            >
              {props.codeLabels.SearchBy.map((opt, index) => {
                return(
                  <option key={index} value={opt.Code}>{opt.Label}</option>
                )
              })}
            ...
            <Input
              ...
              className="mr-sm-2"
              onChange={(e) => {
                props.onSearchBarValueChange(e.target.value);
              }}
            />
          </InputGroup>
        </Nav>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    onSearchBarValueChange: (chr) => dispatch({type: 'SEARCH_CHARACTORS', Charactors: chr}),
    onSearchBarTypeChange: (typ) => dispatch({type: 'SEARCH_TYPE', SearchingType: typ})
  };
};
